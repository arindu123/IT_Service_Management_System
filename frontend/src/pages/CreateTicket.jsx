...
function CreateTicket(){
  ...
  const submit=async(event)=>{event.preventDefault();if(form.issueDescription.trim().length<10){setError("Issue description must contain at least 10 characters.");return;}setLoading(true);setError("");try{const request=await hardwareRequestService.create(form);if(files.length)await hardwareRequestService.upload(request._id,files);toast("Hardware request submitted successfully",{tone:"success"});navigate("/tickets");}catch(err){setError(err.response?.data?.message||t("tickets.submitError"));}finally{setLoading(false);}};
  return <Layout><main className="hardware-request-page"><header className="request-page-header"><div><p className="request-kicker">New service request</p><h1>Create Hardware Request</h1><p>Submit a traceable request for IT equipment support, replacement or procurement.</p></div></header><RequestForm form={form} setForm={setForm} user={user} files={files} onFiles={(event)=>setFiles(Array.from(event.target.files||[]))} onSubmit={submit} onCancel={()=>navigate("/tickets")} loading={loading} error={error} enumLabel={enumLabel}/></main></Layout>;
}
