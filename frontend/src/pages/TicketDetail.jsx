...
function TicketDetail(){
  ...
  if(loading)return <Layout><main className="hardware-request-page"><div className="request-loading-skeleton" role="status" aria-label={t('common.loading')}/></main></Layout>;
  if(!request)return <Layout><main className="hardware-request-page"><ErrorState title={t('ticketDetail.notFound')} message={error||t("ticketDetail.notFound")}/></main></Layout>;
  ...
  return <Layout><main className="hardware-request-page">
    <header className="request-page-header"><div><p className="request-kicker">{t("tickets.hardwareRequest")}</p><h1>{request.ticketId}</h1><p>{request.issueDescription}</p></div></header>
    {error&&<ErrorState title={t('common.noActionAvailable')} message={error}/>} 
    <div className="request-details-layout"><div className="request-details-stack"><RequestDetails request={request} enumLabel={enumLabel} formatDate={formatDate}/><RequestTimeline history={request.statusHistory} enumLabel={enumLabel} formatDateTime={formatDateTime}/><RequestEvidence files={request.attachments} formatDate={formatDate} loading={actionLoading} canDelete={(file)=>canDeleteEvidence(file,user)} onView={openEvidence} onDownload={downloadEvidence} onDelete={setEvidenceToDelete}/></div>    <aside className="request-details-sidebar">{canWorkflow&&<ApprovalPanel key={`${request._id}-${request.updatedAt}`} request={request} enumLabel={enumLabel} loading={actionLoading} canApprove={canApprove} technicians={technicians} onSave={(payload)=>runUpdate(()=>hardwareRequestService.updateStatus(id,payload),t("tickets.workflowUpdated"))} onApprove={(comment)=>runUpdate(()=>hardwareRequestService.acknowledge(id,comment),t("tickets.requestApproved"))} onReject={(comment)=>runUpdate(()=>hardwareRequestService.reject(id,comment),t("tickets.requestRejected"))} onAssign={canApprove?(technicianId)=>runUpdate(()=>hardwareRequestService.assign(id,technicianId),t("tickets.technicianAssigned")):undefined}/>}<RequestActions canEdit={canEdit} loading={actionLoading} onEdit={()=>setEditOpen(true)} onDelete={()=>setDeleteOpen(true)} onBack={()=>navigate("/tickets")}/></aside></div>
    <Modal open={editOpen} onClose={()=>setEditOpen(false)} title={t("tickets.updateRequestTitle",{ticketId:request.ticketId})} size="lg">{editForm&&<RequestForm mode="edit" form={editForm} setForm={setEditForm} user={user} onSubmit={updateRequest} onCancel={()=>setEditOpen(false)} loading={actionLoading} error="" enumLabel={enumLabel}/>}</Modal>
    <ConfirmationDialog open={deleteOpen} onClose={()=>setDeleteOpen(false)} onConfirm={deleteRequest} danger loading={actionLoading} title={t("tickets.deleteHardwareRequest")} message={t("tickets.deleteRequestConfirmMsg")} confirmLabel={t("tickets.deleteRequestConfirmLabel")}/>
    <ConfirmationDialog open={Boolean(evidenceToDelete)} onClose={()=>setEvidenceToDelete(null)} onConfirm={deleteEvidence} danger loading={actionLoading} title={t("tickets.deleteAttachmentTitle")} message={evidenceToDelete?`${evidenceToDelete.originalName}? ${t("tickets.deleteAttachmentMsg")}`:""} confirmLabel={t("tickets.deleteAttachmentLabel")}/>
  </main></Layout>;
}
