  ...
  useEffect(() => {
    const checkToken = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.message || t("auth.resetLinkInvalid"));
      } finally {
        setChecking(false);
      }
    };
        ...
        <div className="grid w-full overflow-hidden rounded-[22px] bg-[#f8fbff] p-5 shadow-[0_24px_70px_rgba(17,24,39,0.14)] lg:min-h-[760px] lg:grid-cols-[0.78fr_1.22fr] lg:gap-8 lg:p-7">
                ...
                <>
                  <div className="mt-5 rounded-[18px] border border-[#dbe6ff] bg-white p-4 shadow-sm">
                    ...
                    <p className="mt-1 text-sm font-semibold text-slate-600">
                      {tokenInfo.user.employeeId} | {tokenInfo.user.emailMasked || t("common.emailNotAvailable")}
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="mt-5 space-y-4">
                    ...
                    >
                      {saving ? t("common.saving") : t("common.resetPasswordArrow")}
                    </Button>
                  </form>
                </>
              )}
          ...
          <section className="relative hidden min-h-[720px] overflow-hidden rounded-[18px] bg-[#10202b] lg:block">
            ...
            <div className="absolute left-[13%] top-[28%] w-[76%] rounded-[18px] bg-[#eaf7fb] p-6 shadow-[0_30px_60px_rgba(2,8,23,0.28)]">
              <p className="text-lg font-semibold text-slate-950">{t("auth.passwordReset")}</p>
              <p className="mt-1 text-3xl font-black tracking-tight text-slate-950">{t("auth.approved")}</p>
              <div className="mt-7 rounded-[18px] bg-white p-5">
                {[t("auth.validatedResetLink"), t("auth.createPasswordStep"), t("auth.signInWithEmployeeId")].map((item, index) => (
