...
export default function RequestOverview({ stages, total }) {
  ...
  return (
    <Card>
      <CardHeader title={t('dashboardPage.requestStatusSummary')} description={t('dashboardPage.requestStatusDesc')} />
      <CardBody>
          ...
          {stages.map((stage) => {
            ...
            return (
              <div className="dashboard-progress-row" key={stage.label}>
                <div><StatusBadge status={stage.status} label={stage.label} /><span>{stage.unavailable ? t('dashboardPage.notReported') : stage.value}</span></div>
